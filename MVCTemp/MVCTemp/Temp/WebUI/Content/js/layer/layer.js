...
this.layer = {
	...
	}, alert: function(alertMsg , isHaveImg, alertTit , alertYes){	//普通对话框，类似系统默认的alert()
		return $.layer({
			dialog : {msg : alertMsg, type : (isHaveImg === true || isHaveImg === undefined) ? 3 : -1, yes : alertYes},
			title : alertTit,
			area: ['auto', 'auto']
		});
	}, confirm: function(conMsg  , conYes , conTit , conNo , isHaveImg){ //询问框，类似系统默认的confirm()
		return $.layer({
			dialog : {msg : conMsg, type : (isHaveImg === true || isHaveImg === undefined) ? 3 : -1, btns : 2, yes : conYes, no : conNo},
			title : conTit
		}); 
	...
	}, warning : function(msgText, callback, isHaveImg){
		return $.layer({
			dialog : { msg:msgText, type : (isHaveImg === true || isHaveImg === undefined) ? 0 : -1, yes : callback}
		});
	...
	}, load: function(loadTime , loadgif , loadShade, waitMsg){
		var border = true;
		loadgif === 3 && (border = false);
		return $.layer({
			time : loadTime,
			shade : [0.3, '#000', loadShade],
			loading : {type : loadgif, msg: waitMsg},
			border :[10,0.3,'#000',border],
			type : 3,
			title : ['',false],
			closeBtn : [0 , false]
		}); 
	}, followTips: function (html, follow) {
		return $.layer({
			type: 5,
			followTips: { msg: html, follow: follow }
		});
    }
};
...
var Class = function(setings){	
	...
	this.config.followTips = $.extend({}, config.followTips, setings.followTips);
	this.creat();
};
...
Class.pt.space = function(html){
	...
	frame = [
		...
		'<div class="xubox_loadingMain"><span class="xubox_loading xubox_loading_'+ config.loading.type +'"></span><span class="xubox_loadmsg">'+ config.loading.msg +'</span></div>',
		'<div class="xubox_tips" style="'+ config.tips.style[0] +'"><div class="xubox_tipsMsg">'+ config.tips.msg +'</div><i class="layerTipsG"></i></div>',
		'<div class="xubox_followtips">' + config.followTips.msg + '</div>'
	],
	...
	var title = '', closebtn = '', borderStyle = "z-index:"+ (zIndex-1) +"; top:-"+ config.border[0] +"px; left:-"+ config.border[0] +"px;";
...
Class.pt.creat = function(){
	...
	switch(config.type){
		...
		case 3:
			 ...
			 $('.xubox_loadingMain')[0] && layer.close($('.xubox_loadingMain').parents('.'+dom.lay).attr('times'));
			 setSpace();
			 body.append(space[1]);
		break;
		case 4:
		...
		case 5:
			...
			config.shade = false;
			config.closeBtn = ['', false];
			$('.xubox_followtips')[0] && layer.close($('.xubox_followtips').parents('.'+dom.lay).attr('times'));
			setSpace();
			body.append(space[1]);
			break;
		default : 
...
Class.pt.set = function(times){
	...
	switch(config.type){
		...
		break;
		case 5:
			var fowtip = $(config.followTips.follow);
			var ftipHight = fowtip.outerHeight();
			var ftipWidth = fowtip.outerWidth();
			var ftipleft = fowtip.offset().left;
			var ftiptop = fowtip.offset().top;
			...
			$(document).mousemove(function(e){
				...
				if(mouseX < ftipleft || mouseX > (ftipleft+ ftipWidth) ||  mouseY < ftiptop || mouseY > (ftipHight + ftiptop)){
					$(".xubox_followtips").parents('.xubox_layer').css("display","none");
				}
				else{
					e.preventDefault();
					if (ftipleft <= mouseX <= (ftipleft + ftipWidth)
						&& ftiptop <= mouseY <= (ftipHight + ftiptop)) {
						$(".xubox_followtips").parents('.xubox_layer').css("display","block");
						layerE.css({ top: mouseY + 10, left: mouseX + 40 });
					}
				}
			});
        break;		
...
Class.pt.autoArea = function(times){
	...
	switch(config.type){
		...
		break;
		case 3:
			var load = $(".xubox_loadingMain"), btnHeight = 0; var outHeight = load.outerHeight() * 2 + 15; 
		break;
	};
	
	(config.area[1] === 'auto') && (config.type == 3 ? '' : layerMian.css({height: titHeight + outHeight + btnHeight}));
...
Class.pt.openLayer = function(){
	...
	layer.loadClose = function(){
		var parent = $('.xubox_loadingMain').parents('.'+dom.lay),
